CLASS zcl_170423842_rest_client_02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_170423842_rest_client_02 IMPLEMENTATION.
...
*    io_out->write( user_delete( ) ).
  ...
  METHOD user_get.
    rv_text = execute_request(
      iv_method = 'GET'
      iv_url    = gc_base_url && '/user/demo_170423842' ).
  ENDMETHOD.
  ...
  METHOD build_user_json.
    rv_json =
      '{' &&
      '"id": 28,' &&
      '"username": "demo_170423842",' &&
      '"firstName": "yusufbilal",' &&
      '"lastName": "ustad",' &&
